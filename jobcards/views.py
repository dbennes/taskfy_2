...
def dashboard(request):
    ...
    # prepare bar data: not checked by discipline
    not_checked_qs = (
        JobCard.objects
    ...
    # prepare pie data: total jobcards per discipline
    total_by_disc = (
        JobCard.objects
        .values('discipline')
        .annotate(count=Count('id'))
...
def edit_jobcard(request, jobcard_id=None):
    ...
    engineering_list = EngineeringBase.objects.filter(tag=job.tag) if job else []
...
def import_materials(request):
    if request.method == "POST":
        ...
        required_columns = [
            'job_card_number', 'description', # ...complete...
        ]
...
def import_jobcard(request):
    if request.method == "POST":
        ...
        for _, row in df.iterrows():
            ...
            if overwrite and jobcard_exist:
                jobcard = JobCard.objects.get(job_card_number=job_card_number)
                for field in required_fields:
                    setattr(jobcard, field, row[field])
                # Campos automáticos/sistema
                jobcard.rev = str(int(jobcard.rev or 0) + 1)
                jobcard.hot_work_required = "NO"
                jobcard.jobcard_status = "NO CHECKED"
                jobcard.completed = "NO"
                jobcard.prepared_by = request.user.username
                jobcard.date_prepared = timezone.now()
                jobcard.approved_br = ""
                jobcard.date_approved = None
                jobcard.status = "NO"
                jobcard.last_modified_by = request.user.username
                jobcard.last_modified_at = timezone.now()
                jobcard.save()
            else:
                if jobcard_exist and not overwrite:
                    return JsonResponse({'status': 'duplicate', 'message': f"JobCard {job_card_number} already exists. Overwrite?"})

                data = {field: row[field] for field in required_fields}
                data["rev"] = "0"
                data["hot_work_required"] = "NO"
